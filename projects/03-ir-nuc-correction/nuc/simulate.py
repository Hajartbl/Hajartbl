...
(responsivity) and offset, so a perfectly uniform image still looks grainy. We
model each pixel's raw response to an image temperature T as:
...
class IRSensor:
    ...
    def capture_uniform(self, temperature: float, read_noise=1.5) -> np.ndarray:
        """Raw frame of a uniform black-body target at `temperature`."""
        noise = RNG.normal(0, read_noise, self.shape)
        return self.gain * temperature + self.offset + noise

    def capture_image(self, image_temp: np.ndarray, read_noise=1.5) -> np.ndarray:
        """Raw frame of an arbitrary image (per-pixel true temperature map)."""
        noise = RNG.normal(0, read_noise, self.shape)
        return self.gain * image_temp + self.offset + noise


def make_test_image(shape=(96, 128)) -> np.ndarray:
    """A synthetic image: warm disk + gradient background (ground-truth temps)."""
    h, w = shape
